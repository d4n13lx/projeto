...
function exportarParaCSV() {
    ...
    ob_clean(); // Limpa qualquer saída de buffer (como HTML) antes de gerar o arquivo CSV
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="' . $filename . '";');
    $output = fopen("php://output", "w");
    fputcsv($output, array('id', 'Regiao', 'N_Casos', 'N_Mortes', 'Percent_Risco'));
...
// Verifica se o botão de exportar foi clicado
if(isset($_POST["exportarCSV"])) {
    exportarParaCSV();
    exit; // Encerra o script após a exportação para evitar a saída de qualquer HTML adicional
}
?>
...
<svg xmlns="http://www.w3.org/2000/svg" class="dropdown" width="16" height="16" fill="currentColor" class="bi bi-caret-down-fill" viewBox="0 0 16 16">
  ...
  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2M12 4v12m0-12 4 4m-4-4L8 8"/>
    ...
    <div class="h1">
        <div class="content">
        <h1>Exportar de Arquivo CSV</h1>
        </div>
    </div>
    <div class="org">
    
    <h2 style="color:white; text-align:center;">Pressione o botão para exportar o arquivo:</h2>
        <form style="text-align:center"action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            <input type="submit" value="Exportar Dados" name="exportarCSV">
        </form>
    </div>
