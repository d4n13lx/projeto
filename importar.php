...
if(isset($_POST["submit"])) {
    ...
    if(isset($_FILES["fileToUpload"])) {
        ...
        // Verifica se o arquivo é realmente um arquivo CSV
        if($fileType != "csv") {
            echo "Desculpe, apenas arquivos CSV são permitidos.";
            $uploadOk = 0;
        }

        // Se houver algum erro no upload, exibe o erro
        if ($uploadOk == 0) {
            echo "Erro ao fazer upload do arquivo.";
        // Se tudo estiver OK, tenta fazer o upload
        } else {
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                echo "<h3>O arquivo ". htmlspecialchars( basename( $_FILES["fileToUpload"]["name"])). " foi enviado com sucesso.</h3>";
                ...
                if (($handle = fopen($csv_file, "r")) !== FALSE) {
                    // Ignora a primeira linha (cabeçalho das colunas)
                    fgetcsv($handle, 1000, ",");
                    ...
                    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                        ...
                        if (count($data) == 5) {
                            ...
                            $id = $data[0];
                            $check_query = "SELECT id FROM regioes_risco WHERE id = '$id'";
                            $result = conectarBanco()->query($check_query);
                            if ($result->num_rows > 0) {
                                echo "<h3><br>ID duplicado encontrado: $id </h3>";
                            } else {
                                // Insere os dados no banco de dados
                                $sql = "INSERT INTO regioes_risco (id, Regiao, N_casos, N_mortes, Percent_Risco) VALUES ('$data[0]', '$data[1]', '$data[2]', '$data[3]', '$data[4]')";

                                if (conectarBanco()->query($sql) === TRUE) {
                                    echo "<h3><br>Registro inserido com sucesso.</h3>";
                                } else {
                                    echo "<h3><br>Erro ao inserir registro: " . $sql . "<br>" . conectarBanco()->error . "</h3>";
                                }
                            }
                        } else {
                            echo "<h3><br>Erro: número incorreto de colunas.</h3>";
                        }
                    }
                    fclose($handle);
                } else {
                    echo "<h3><br>Erro ao abrir o arquivo CSV.</h3>";
                }
            } else {
                echo "<h3>Erro ao fazer upload do arquivo.</h3>";
            }
        }
    }
}
...
<svg xmlns="http://www.w3.org/2000/svg" class="dropdown" width="16" height="16" fill="currentColor" class="bi bi-caret-down-fill" viewBox="0 0 16 16">
  ...
  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2M12 4v12m0-12 4 4m-4-4L8 8"/>
    ...
    <h2 style="color:white">Selecione o arquivo CSV para upload:</h2>
        <input type="file" name="fileToUpload" id="fileToUpload">
        <input type="submit" value="Upload Arquivo CSV" name="submit">
    </form>
